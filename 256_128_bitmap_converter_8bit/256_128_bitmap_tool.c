...
/* Row alignment in bits (for example 8, 16, or 32) */
#define ALIGN_BITS  8

/* 1: MSB represents the leftmost pixel, 0: LSB represents the leftmost pixel */
#define BIT_MSB_LEFT  1

/* 1: In the source data, bit=1 means white; 0: bit=1 means black */
...
/* ================================================
   ...
   You don’t need to load it from a file.
   Just paste the hexadecimal values directly, for example:
   0xFF, 0x00, 0xAA, 0x55, ...
   ================================================ */
...
int main(void)
{
    ...
    /* Convert the array into a byte stream (handle uint16_t correctly) */
    if (elem_size == 1) {
    ...
    for (unsigned y = 0; y < IMG_H; ++y) {
        ...
        for (unsigned x = 0; x < IMG_W; ++x) {
            ...
            const char *token = white ? "\"11111111\"" : "\"00000000\"";
            printf("%s", token);
            if (x < IMG_W - 1) printf(", ");
        }
        printf(",\n\n");
